...
/**
 * The404MasterChef is the master of Reward The404(TH3) = ReTk(reTk)
 * 
 * Note that it's ownable and the owner wields tremendous power.
 *
 * Have fun reading it. Hopefully it's bug-free. God bless.
 */
contract The404MasterChef is Ownable, ReentrancyGuard {
    using SafeMath for uint256;
    ...
    constructor (
        ...
        require(_reTkPerBlock <= MAXIMUM_EMISSON_RATE.mul(10**rewardTokenDecimals), "Emission rate exceeds limit");
    ...
    function payOrLockupPendingReTk(uint256 _pid) internal {
        ...
        if (canHarvest(_pid, msg.sender)) {
            if (pending > 0 || user.rewardLockedUp > 0) {
                ...
                // send rewards
                safeReTkTransfer(msg.sender, totalRewards);
                // reset lockup
    ...
    function safeReTkTransfer(address _to, uint256 _amount) internal {
        IRETK(address(reTk)).mint(_to, _amount);
    }
